...
function headers(): HeadersInit {
  const js = env("LINKEDIN_JSESSIONID");
  return {
    ...
    cookie: `li_at=${env("LINKEDIN_LI_AT")}; JSESSIONID=${js}`,
  };
}
async function get<T extends VoyagerResponse>(path: string): Promise<T> {
  ...
  try {
    ...
    if (e instanceof LinkedInUpstreamError) throw e;
    if (e instanceof Error && e.name === "AbortError")
      throw new LinkedInUpstreamError("LinkedIn request timed out", 504);
    throw e;
  } finally {
    clearTimeout(timer);
  }
...
export class LinkedInClient {
  ...
  getProfile(publicIdentifier: string) {
    ...
    return get(`/voyager/api/identity/dash/profiles?${q.toString()}`);
  }
}
